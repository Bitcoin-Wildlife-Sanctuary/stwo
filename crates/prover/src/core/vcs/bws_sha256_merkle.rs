...
mod tests {
    ...
    type TestData = (
        ...
        MerkleDecommitment<BWSSha256MerkleHasher>,
        Vec<Vec<BaseField>>,
        MerkleVerifier<BWSSha256MerkleHasher>,
    );
    fn prepare_merkle() -> TestData {
        ...
        let cols = log_sizes
            ...
            .collect_vec();
        let merkle =
            MerkleProver::<CpuBackend, BWSSha256MerkleHasher>::commit(cols.iter().collect_vec());
    ...
    fn test_merkle_invalid_witness() {
        ...
        decommitment.hash_witness[20] = BWSSha256Hash::default();
    ...
    fn test_merkle_witness_too_long() {
        let (queries, mut decommitment, values, verifier) = prepare_merkle();
        decommitment.hash_witness.push(BWSSha256Hash::default());
